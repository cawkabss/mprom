...
export const parsing = (id) => {
    return (dispatch, getState) => {
        ...
        const {parseQuery, maxCount, timeOut} = getState().search.settings;
...
export const saveResultProduct = () => {
    return (dispatch, getState) => {
        ...
        const resultProduct = getState().search.resultProduct;
        resultProduct.isDone = true;

        axios.put(`/api/products/${resultProduct._id}`, resultProduct)
            .then(res => {
                dispatch(requestResponse(actionTypes.PARSING_SAVE_RESULT_PRODUCT_SUCCESS, res.data));

                dispatch(parsing(resultProduct.provider))
            })
